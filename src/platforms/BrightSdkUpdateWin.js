...
class BrightSdkUpdateWin extends BrightSdkUpdateBase {
    ...
    async assign_appdir() {
        ...
        if (found.length === 1) {
            ...
            this.csproj_fname = path.isAbsolute(chosen) ? chosen : path.join(this.workdir, chosen);
        } else {
    ...
    get_sdk_files() {
        ...
        if (this.sdk_service_fname && this.sdk_service_dir) {
            const svc_dst = path.join(this.sdk_service_dir, path.basename(this.sdk_service_fname));
    ...
    async update_sdk_files() {
        ...
        const libs_dir_rel = path.relative(csproj_dir, path.join(this.workdir, this.libs_dir));
        const patched = patch_csproj(this.csproj_fname, hint_path, libs_dir_rel);
        if (patched) {
            this.print(
                `✔ Patched ${path.relative(this.workdir, this.csproj_fname)} (added lum_sdk reference + brd_config.json)`,
            );
        } else {
