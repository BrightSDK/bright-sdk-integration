...
class BrightSdkUpdateBase {
    ...
    get_workdir_excludes() {
        return ['.git', '.github', '.sdk', '.vscode', '.idea', '.build', 'node_modules'];
    }
    print_greeting() {
    ...
    async assign_sdk_service_dir() {
        ...
        this.sdk_service_dir = await this.get_value(
            'SDK Service dir',
            sdk_service_dir_def,
            this.config.sdk_service_dir,
            {
                selectable: true,
                dir: sdk_service_dir_def
                    ? path.join(this.workdir, sdk_service_dir_def)
                    : this.workdir,
            },
        );
    }
    ...
    async check_sdk_ver() {
        if (this.config.sdk_ver_prev && this.config.sdk_ver_prev === this.sdk_ver) {
            this.print('SDK is already of the latest version.');
            const force =
                this.opt.interactive && (await this.get_value('Force update? (y/n)', 'n'));
            if (force != 'y') {
                return;
            }
        }
    ...
    async assign_sdk_url() {
        ...
        if (!default_sdk_url) {
            throw new Error(
                'SDK URL mask not configured. Please add defaults.sdk_url_mask to config.json',
            );
        }
        this.sdk_url_mask = await this.get_value(
            'SDK URL mask',
            default_sdk_url,
            this.config.sdk_url,
        );
    ...
    async download_sdk() {
        if (fs.existsSync(this.sdk_dir) && this.sdk_versions[this.sdk_ver]) {
            this.print(
                `✔ Using cached SDK version ${
                    this.sdk_ver
                } downloaded on ${this.sdk_versions[this.sdk_ver].date}`,
            );
        } else {
            const platform_key = this.get_platform_version_key();
    ...
    build_config() {
        ...
        this.config_fnames =
            this.opt.config_fnames || (this.opt.config_fname && [this.opt.config_fname]);
        if (this.opt.config) {
    ...
    async assign_libs_dir() {
        ...
        this.libs_dir = await this.get_value(
            this.libs_dir_prompt(),
            libs_dir_def,
            libs_dir_config,
    ...
    get_git_commit_files() {
        const result = [];
        if (this.brd_api_dst_fname) {
            result.push(this.brd_api_dst_fname);
        }
    ...
    get_git_commit_commands(next_config_fname) {
        const sdk_ver_from =
            this.config.sdk_ver_prev && this.config.sdk_ver_prev != this.sdk_ver
...
`);
    ...
    save_config() {
        if (!this.opt.config) {
            ...
            const next_config_fname_relative = this.workdir_relative_path(
                get_config_fname(this.workdir),
            );
