...
const { read_json, download_from_url, set_json_props } = lib;
...
class BrightSdkUpdateWeb extends BrightSdkUpdateBase {
    constructor(opt) {
        ...
        if (!this.brd_api_name && fs.existsSync(this.config_path)) {
            throw new Error(
                'API filename not configured. Please add files.api_name to config.json',
            );
        }
        this.js_ext = '.js';
        this.appid = null;
    ...
    async get_libs_dir_def() {
        // doesn't use parent implementation
        let def_value;
        const existing = await this.search_workdir(`^${this.brd_api_base}(_.+)?\.${this.js_ext}$`); // eslint-disable-line no-useless-escape
        if (existing) {
    ...
    async assign_appdir() {
        if (!this.appdir) {
            this.appdir = await this.get_value(
    ...
    async assign_use_helper() {
        const use_helper_yes_no = await this.get_value(
            'Use BrightSDK Integration Helper? (y/n)',
            'y',
            this.config.use_helper && 'y',
        );
    ...
    async assign_brd_api_dest_name() {
        this.brd_api_dst_name = this.brd_api_name.replace(
            this.js_ext,
    ...
    async assign_brd_api_helper_name() {
        ...
        if (!this.brd_api_helper_name) {
            throw new Error(
                'Helper filename not configured. Please add files.helper_name to config.json',
            );
        }
    }
    async assign_brd_api_helper_filename() {
        ...
        if (this.app_config.files?.helper_name_local) {
            this.brd_api_helper_fname = path.join(
                __dirname,
                '..',
                this.app_config.files.helper_name_local,
            );
        } else {
            this.brd_api_helper_fname = path.join(
                __dirname,
    ...
    get_sdk_files() {
        ...
        if (this.use_helper) {
            files.push([this.brd_api_helper_fname, this.brd_api_helper_dst_fname]);
        }
        return files;
    }
    update_brd_api() {
        let brd_api_name_prev,
            brd_api_fname_prev = 'none';
        if ((brd_api_name_prev = this.update_index_ref(this.index_fname, this.brd_api_dst_name))) {
            ...
            if (!this.is_web_hosted && brd_api_fname_prev != this.brd_api_dst_fname) {
                const abs = path.isAbsolute(brd_api_fname_prev)
                    ? brd_api_fname_prev
                    : path.join(this.workdir, brd_api_fname_prev);
...
const process_web = async (opt = {}) => {
    ...
    if (!platform) {
        throw new Error(
            `Unsupported platform: ${opt.platform}. Supported: ${Object.keys(platforms).join(', ')}`,
        );
    }
