...
const create_readline_interface = () =>
        ...
        new Promise((resolve, reject) => {
            ...
            rl.input.on('keypress', async (str, key) => {
                if (key.name == 'up') {
                    ...
                    if (
                        key.name == 'right' &&
                        fs.lstatSync(path.join(current_dir, files[current_index])).isDirectory()
                    ) {
                        ...
                        current_dir = cwd; // eslint-disable-line no-undef
                        select(0);
                        await read_dir();
                    }
                }
            ...
            const read_dir = async () => {
                try {
                    const f = await readdir(current_dir);
                    files = f;
                    if (!opt.file) {
...
if (require.main === module) {
    prompt.then(result => print(`Result: ${result}`)).catch(err => console.error(`Error: ${err}`));
}
