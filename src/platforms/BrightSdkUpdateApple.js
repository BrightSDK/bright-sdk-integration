...
class BrightSdkUpdateApple extends BrightSdkUpdateBase {
    ...
    async assign_appdir() {
        ...
        if (found.length === 1) {
            ...
            this.xcodeproj_dir = path.isAbsolute(chosen) ? chosen : path.join(this.workdir, chosen);
        } else {
...
class BrightSdkUpdateAppleDesktop extends BrightSdkUpdateApple {
    ...
    async update_sdk_files() {
        ...
        if (!fs.existsSync(entitlements_src) || !fs.existsSync(resign_src)) {
            this.print(
                '⚠ net_updater.entitlements or resign_net_updater.sh not found in SDK — ' +
                    'skipping Resign build phase. Add it manually from the SDK docs.',
            );
        } else {
            const entitlements_dst = path.join(
                this.workdir,
                this.libs_dir,
            ...
            lib_xcode.set_build_setting(project, 'NET_UPDATER_ENTITLEMENTS', entitlements_rel);
        }
